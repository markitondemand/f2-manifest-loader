...
module.exports = function (source) {
	...
	var appStyles = loaderOptions.styles;
	...
	var filename = loaderOptions.filename || appid + '_manifest';

	var template = fs.readFileSync(path.join(__dirname, './template.js'), 'utf8');
	var output = mustache.render(template, templateOptions);
	var outputPath = path.join(normalizePath(loaderOptions.dest), filename + '.js');
	...
	var outputJsonPath = path.join(normalizePath(loaderOptions.dest), filename + '.json');
